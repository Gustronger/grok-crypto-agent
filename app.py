...
# Custom CSS - Premium Powerful Look
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem; 
    ...
    .conviction-high { color: #00ff9d; font-weight: bold; }
    .conviction-medium { color: #ffaa00; font-weight: bold; }
    .conviction-low { color: #ff4b4b; font-weight: bold; }
...
def main():
    ...
    st.markdown('<p class="sub-header">Professional Smart Money Trading Assistant • Powered by Advanced SMC + Multi-TF Analysis</p>', unsafe_allow_html=True)
    st.markdown("---")
            ...
            if st.button(f"🚀 ANALISA LENGKAP {selected_symbol}", type="primary", use_container_width=True):
                with st.spinner(f"🤖 GROK CRYPTO AGENT menganalisa {selected_symbol}... (Structure + FVG + Multi-TF + Liquidity + Scoring)"):
                    ...
                    if df is not None and len(df) > 30:
                        ...
                        # Advanced Setup Quality Score - Premium Display
                        ...
                        with col_score1:
                            st.markdown(f"""
                            <div class="premium-card" style="text-align: center; padding: 20px;">
                                <div style="font-size: 1rem; color: #aaaaaa; margin-bottom: 5px;">SETUP QUALITY SCORE</div>
                                <div class="score-big">{adv_score}</div>
                                <div style="font-size: 1.1rem; margin-top: 5px;">/ 100</div>
                            </div>
                            """, unsafe_allow_html=True)
                        
                        with col_score2:
                            conv_color = "#00ff9d" if conviction in ["Very High", "High"] else "#ffaa00" if conviction == "Medium" else "#ff4b4b"
                            st.markdown(f"""
                            <div class="premium-card">
                                <div style="font-size: 0.95rem; color: #aaaaaa;">CONVICTION LEVEL</div>
                                <div style="font-size: 2.1rem; font-weight: 700; color: {conv_color};">{conviction}</div>
                                <div style="margin-top: 8px; font-size: 0.9rem; color: #cccccc;">{adv_reasons[0] if adv_reasons else "Setup terdeteksi"}</div>
                            </div>
                            """, unsafe_allow_html=True)
                        
                        with st.expander("📊 Lihat Breakdown Score Lengkap"):
